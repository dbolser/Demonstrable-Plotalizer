...
- [x] **Shareable state links** ([#43](https://github.com/dbolser/Demonstrable-Plotalizer/issues/43)) — serialize view state (data URL, column visibility/order/scales, color-by, filters, selection) into a compact URL fragment. For uploaded files, share state minus data with a "load your copy" prompt. Depends on load-from-URL. *Shipped as `#view=` (schema in `src/utils/viewState.ts`); brush selection and PCA columns deliberately not captured (transient / derived).*
